...
function UserManagementContent() {
  ...
  const handleOpenRequest = (req: UserRequest) => {
    const row: User = {
      ...
      status: "pending",
      role: ((req.currentRole?.name ?? "guest").toLowerCase() === "developer"
        ? "dev"
        : (req.currentRole?.name ?? "guest").toLowerCase()) as User["role"],
    };
    setCurrentRow(row);
    setOpen(
      req.requestType === "Account Registration" ? "approve" : "approveReset",
    );
  };
          ...
          <Drawer direction="right">
            ...
            <DrawerContent direction="right">
              <DrawerHeader>
                ...
                <DrawerDescription>
                  Review and manage pending user requests.
                </DrawerDescription>
              </DrawerHeader>
              <div className="px-4">
                {loading ? (
